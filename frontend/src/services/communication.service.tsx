...
export const deleteAllLogsByUser = async (
  ...
  try {
    const response = await axiosInstance.delete(`${BASE_API_URL}/logs/user/${userId}`, {
      data: data,
      headers: {
        Authorization: `Bearer ${await user?.getIdToken(true)}`,
      },
...
export const fetchLogsByUser = async (data: any, user: User) => {
  try {
    const response = await axiosInstance.post(`${BASE_API_URL}/logs/user`, data, {
      headers: {
        Authorization: `Bearer ${await user?.getIdToken(true)}`,
      },
    });
...
export const fetchNLPOutput = async (data: any) => {
  try {
    const response = await axiosInstance.post(`${BASE_API_URL}/nlp/output`, data);
    return response.data; 
  } catch (err) {
    throw err; 
  }
...
export const fetchSLROutput = async (formData: FormData) : Promise<any> => {
  try {
    ...
    const response = await axiosInstance.post(`${BASE_API_URL}/slr/output`, formData, {
      headers: {
