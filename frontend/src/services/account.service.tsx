...
export const FetchGoogleData = async (token: string) => {
  try {
    const response = await axiosInstance.get(GOOGLE.GOOGLELAPIS, {
      headers: {
...
export const LoginUser = async (data: any) => {
  try {
    const loginUser = await axiosInstance.post(`${BASE_API_URL}/users/login`, data);
    return loginUser;
  } catch (err) {
    throw err;
  }
...
export const LogoutUser = async (data: any) => {
  try {
    const logoutUser = await axiosInstance.post(`${BASE_API_URL}/users/logout`, data);
    return logoutUser;
  } catch (err) {
    throw err;
  }
...
export const GetUserByEmail = async (email: string | null, user: User) => {
  try {
    const response = await axiosInstance.get(`${BASE_API_URL}/users/${email}/profile`, {
      headers: {
        Authorization: `Bearer ${await user?.getIdToken(true)}`,
      },
    });
...
export const FetchAllCountries = async (user: User) => {
  try {
    const response = await axiosInstance.get(`${BASE_API_URL}/users/countries`, {
      headers: {
        Authorization: `Bearer ${await user?.getIdToken(true)}`,
      },
    });
...
export const FetchAllUsers = async (user: User | null) => {
  try {
    const response = await axiosInstance.get(`${BASE_API_URL}/users/all-users`, {
      headers: {
        Authorization: `Bearer ${await user?.getIdToken(true)}`,
      },
    });
...
export const FetchAllDataset = async (user: User | null) => {
  try {
    const response = await axiosInstance.get(`${BASE_API_URL}/users/all-datasets`, {
      headers: {
        Authorization: `Bearer ${await user?.getIdToken(true)}`,
      },
    });
